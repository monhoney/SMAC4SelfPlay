...
from util.serialization_utils import convert_json
...
class EpochLogger(Logger):
    ...
    def log_wandb(self, info_list):
        if self.use_wandb == False:
            return

        wandb_info = dict()

        for key, with_min_and_max, average_only in info_list:
            v = self.epoch_dict[key]
            vals = np.concatenate(v) if isinstance(v[0], np.ndarray) and len(v[0].shape)>0 else v
            stats = mpi_statistics_scalar(vals, with_min_and_max=with_min_and_max)
            if average_only == True:
                wandb_info[key] = stats[0]
            else:
                wandb_info['Average' + key] = stats[0]

            if not(average_only):
                wandb_info['Std'+key] = stats[1]

            if with_min_and_max:
                wandb_info['Max'+key] = stats[3]
                wandb_info['Min'+key] = stats[2]

        super().log_wandb(wandb_info)
